                ...
                <div class="w-full max-w-[860px] flex flex-col gap-6">
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 p-8 mt-6">
                        ...
                        <?php if (!$hasActiveStay && !$hasPendingRequest && empty($stayHistory)) : ?>
                            <p class="text-sm text-slate-600 dark:text-slate-300">No hay estancias registradas aún / No internships registered yet.</p>
                        <?php else : ?>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <?php if ($hasPendingRequest) : ?>
                                    <article class="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-5">
                                        ...
                                        <p class="mt-3 inline-flex rounded-full bg-amber-200 dark:bg-amber-900 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-amber-900 dark:text-amber-100">Pendiente de aprobación / Pending approval</p>
                                    </article>
                                <?php endif; ?>
        ...
        (() => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    btnText.textContent = lang === 'en' ? 'Save new password' : 'Guardar nueva contraseña / Save new password';
                }
            });
        })();
    </script>
