...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_all') {
  ...
  if ($isAdmin && is_array($editRows) && isset($editRows['cas_nr']) && is_array($editRows['cas_nr'])) {
    foreach ($editRows['cas_nr'] as $idRaw => $casRaw) {
      ...
      $onLoan = !empty($editRows['prestamo_on_loan'][$id]) ? 1 : 0;
      $gpriv = (int)($editRows['acceso_grupo_privado_id'][$id] ?? 0);
      $gprivValue = ($acc === 'privado' && $gpriv > 0) ? $gpriv : null;
      if ($cas !== '' && $prov !== '' && $nom !== '' && $owner > 0 && $loc !== '') {
        if ($hasQty) {
          $up = $mysqli->prepare('UPDATE chemicals SET cas_nr=?, proveedor=?, nombre=?, formato_size=?, unidades=?, grupo_id=?, localizacion=?, cantidad=?, acceso=?, prestamo_on_loan=?, acceso_grupo_privado_id=? WHERE id=?');
          if ($up) { $up->bind_param('sssssisdsiii', $cas, $prov, $nom, $fmt, $units, $owner, $loc, $qty, $acc, $onLoan, $gprivValue, $id); $up->execute(); $up->close(); }
        } else {
          $up = $mysqli->prepare('UPDATE chemicals SET cas_nr=?, proveedor=?, nombre=?, formato_size=?, unidades=?, grupo_id=?, localizacion=?, acceso=?, prestamo_on_loan=?, acceso_grupo_privado_id=? WHERE id=?');
          if ($up) { $up->bind_param('sssssisdsii', $cas, $prov, $nom, $fmt, $units, $owner, $loc, $acc, $onLoan, $gprivValue, $id); $up->execute(); $up->close(); }
        }
      }
    }
  } elseif (is_array($editRows) && isset($editRows['localizacion']) && is_array($editRows['localizacion'])) {
    foreach ($editRows['localizacion'] as $idRaw => $locRaw) {
      $id = (int)$idRaw; if ($id <= 0) continue;
      $loc = trim((string)$locRaw);
      $hasQty = isset($quantities[$id]);
      $qty = $hasQty ? (float)$quantities[$id] : 0.0;
      if ($hasQty) {
        ...
        $up = $mysqli->prepare("UPDATE chemicals SET localizacion=? WHERE id=? AND grupo_id=? AND (acceso='publico' OR (acceso='privado' AND acceso_grupo_privado_id=?))");
        if ($up) { $up->bind_param('siii', $loc, $id, $userGroupId, $userGroupId); $up->execute(); $up->close(); }
      }
    }
  ...
  if ($isAdmin && isset($_POST['new_rows']) && is_array($_POST['new_rows'])) {
    ...
    for ($i = 0; $i < $total; $i++) {
      ...
      $owner = (int)($nr['grupo_id'][$i] ?? 0); $loc = trim((string)($nr['localizacion'][$i] ?? '')); $cant = (float)($nr['cantidad'][$i] ?? 0);
      ...
      $onLoan = !empty($nr['prestamo_on_loan'][$i]) ? 1 : 0;
      $gpv = ($acc === 'privado' && $gp > 0) ? $gp : null;
      if ($cas === '' || $prov === '' || $nom === '' || $owner <= 0 || $loc === '') continue;
      $in = $mysqli->prepare('INSERT INTO chemicals (cas_nr, proveedor, nombre, formato_size, unidades, grupo_id, localizacion, cantidad, acceso, prestamo_on_loan, acceso_grupo_privado_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
      if ($in) { $in->bind_param('sssssisdsii', $cas, $prov, $nom, $fmt, $units, $owner, $loc, $cant, $acc, $onLoan, $gpv); $in->execute(); $in->close(); }
    }
  }
